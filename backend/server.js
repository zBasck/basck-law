...
app.get('/api/saude', (req, res) => {
  res.json({ ok: true, servico: 'basck-law', versao: '1.0.0', agora: new Date().toISOString() });
});
...
// Frontend estático (DEPOIS das rotas de API para não conflitar)
const frontendDir = path.join(__dirname, '..', 'frontend');
app.use(express.static(frontendDir));

// SPA fallback: só atende arquivos que não começam com /api/
app.get(/^\/(?!api\/).*/, (req, res) => {
  res.sendFile(path.join(frontendDir, 'index.html'), (err) => {
    if (err) res.status(404).json({ erro: 'Página não encontrada' });
  });
});
