...
class IntegracaoModel {
  ...
  static _datajudRequest(tribunal, path, apiKey) {
      ...
      const req = https.request({
        ...
        res.on('end', () => {
          if (res.statusCode >= 200 && res.statusCode < 300) {
            ...
            reject(new Error('DataJud HTTP ' + res.statusCode));
          }
        });
      });
  ...
  static async consultar(usuario_id, integracao_id) {
    ...
    if (integ.tribunal === 'oab') {
      ...
      try {
        ...
        const resp = await this._datajudRequest(integ.tribunal, '/' + integ.tribunal + '/_search', apiKey);
